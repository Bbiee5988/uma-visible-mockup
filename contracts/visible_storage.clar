...
;; It allows organizations to register, store document references, manage access permissions, and maintain an immutable
...
;; Stores registered organizations and their owners
...
(define-private (has-permission
  ...
  (let (
    (business-data (map-get? organizations { business-id: business-id }))
...
(define-public (register-business (business-id (string-ascii 64)) (name (string-ascii 256)))
  (let ((existing-business (map-get? organizations { business-id: business-id })))
...
(define-public (add-document
  ...
  (let ((business-data (map-get? organizations { business-id: business-id })))
    (if (is-none business-data)
...
(define-read-only (get-business-info (business-id (string-ascii 64)))
  (map-get? organizations { business-id: business-id })
)
